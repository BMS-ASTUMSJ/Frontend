...
function AdminProgress() {
  ...
  const handlePublishCp = async (e) => {
    ...
    try {
      ...
      setError(
        err?.response?.data?.message || "Failed to publish CP question.",
      );
    } finally {
  ...
  const handleDeleteContent = async (contentId) => {
    if (!window.confirm("Are you sure you want to remove this learning task?"))
      return;
  ...
  const femaleStudentsCount =
    ...
    studentsProgress.filter((s) => s.student?.gender === "Female").length ||
    0;
  const maleStudentsCount =
    currentBatch?.maleStudents ||
    studentsProgress.filter((s) => s.student?.gender === "Male").length ||
    0;
  ...
  const cpContentList = contentList.filter(
    (c) => c.type === "cp" && (c.week === Number(cpWeek) || !cpWeek),
  );
  const devContentList = contentList.filter(
    (c) => c.type === "dev" && (c.week === Number(devWeek) || !devWeek),
  );
  ...
  const totalExpectedQuestions =
    studentsProgress.reduce((acc, curr) => acc + (curr.total || 0), 0) || 100;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#7A7F85] font-medium">
              • Live Curriculum Hub
            </span>
          </div>
          ...
          <p className="text-xs sm:text-sm text-[#7A7F85]">
            Publish weekly CP problems & Dev lectures, track milestones, and
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-3xl border border-gray-100 bg-white p-6 sm:p-8 shadow-sm lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between border-b border-gray-100 pb-4">
            <div>
              ...
              <p className="text-xs text-[#7A7F85] mt-0.5">
                Aggregated problem solving & video lecture completion
                statistics.
              </p>
            </div>
            <span className="text-xs font-bold text-[#4A7FA7] flex items-center gap-1 bg-[#1A3D63]/5 px-3 py-1 rounded-full">
              <Sparkles className="h-3.5 w-3.5 text-[#1A3D63]" /> Real-Time
              Analytics
            </span>
          </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 pt-2">
            {/* CP Progress Box */}
            <div className="rounded-3xl border border-gray-100 bg-linear-to-br from-[#F6FAFD] to-white p-6 flex items-center justify-between shadow-sm">
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <div className="h-8 w-8 rounded-xl bg-blue-100 text-[#1A3D63] flex items-center justify-center font-bold shadow-sm">
                    <Code2 className="h-4 w-4" />
                  </div>
                  <span className="font-bold text-sm text-[#0A1931]">
                    CP Track
                  </span>
                </div>
          ...
          <div>
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <div className="flex items-center gap-1.5">
                ...
                <h3 className="font-bold text-base text-[#0A1931]">
                  Leaderboard
                </h3>
              </div>
              ...
              <div className="flex rounded-lg bg-gray-100 p-0.5 text-xs font-bold">
                <button
                  ...
                  className={`px-3 py-1 rounded-md transition ${
                    performerTab === "cp"
                      ? "bg-white text-[#1A3D63] shadow-sm font-extrabold"
                      : "text-[#7A7F85]"
                  }`}
                >
                  CP
            ...
            {studentsProgress.length === 0 ? (
              <p className="text-xs text-[#7A7F85] text-center py-8">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-[#7A7F85] font-semibold">
                          {item?.completed}/20
                        </span>
        ...
        <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm space-y-5 hover:shadow-md transition">
          <div className="flex items-center justify-between border-b border-gray-100 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-base text-[#0A1931]">
                Publish CP Progress (Weekly Questions)
              </h3>
            </div>
                          ...
                          <td className="p-2.5">
                            <div className="flex items-center gap-1.5">
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="text-blue-600 hover:underline inline-flex items-center gap-1 truncate max-w-20"
                              >
        ...
        <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm space-y-5 hover:shadow-md transition">
          <div className="flex items-center justify-between border-b border-gray-100 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-base text-[#0A1931]">
                Publish Dev Progress (Weekly Videos)
              </h3>
            </div>
          ...
          <div className="grid gap-6 md:grid-cols-5">
            ...
            <div className="md:col-span-3 space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[#0A1931]">
                  Week {devWeek} - Dev Video Playlist
                </span>
                <span className="rounded-md bg-green-100 text-green-700 px-2 py-0.5 text-[10px] font-bold">
                    ...
                    {devContentList.length === 0 ? (
                      <tr>
                        <td
                          ...
                          className="p-6 text-center text-gray-400"
                        >
                          No Dev video lectures published for Week {devWeek}{" "}
                          yet.
                        </td>
                      </tr>
                    ) : (
                          ...
                          <td className="p-2.5">
                            <div className="flex items-center gap-1.5">
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="text-purple-600 hover:underline inline-flex items-center gap-1 truncate max-w-20"
                              >
